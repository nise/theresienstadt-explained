...
define(['./jquery', './vi2.core.utils'], function ($, Utils) {
    var
        vi2 = window.vi2 || {},
        Vi2 = window.Vi2 || {}
        ;
    ...
    function Timeline(video, options, seek) {
        this.video = video;
        ...
        addTimelineMarkers: function (type, data, timelineSelector) {
            ...
            $.each(data, function (i, val) {
                ...
                if (isNaN(progress) || progress > 1 ) { 
                    console.log('Could not determine timeline progress for annotation ' + val.occ[0] + ' < ' + vi2.observer.player.duration() + ' /= ' + $(timelineSelector).width())
                    return; 
                }

                var sp = $('<a></a>')				
                    .addClass(type + '-timeline-marker')// ttoc
                    .attr('style', 'left:' + progress*100 + '%;')
                    ;
